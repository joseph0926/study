  ...
  <div v-for="thread in threads" :key="thread.id" class="col-large push-top">
    ...
    <div class="post-list">
      <div v-for="post in posts" :key="post.id" class="post">
        <div class="user-info">
          <a href="#" class="user-name">{{ getUser(post).username }}</a>
          <a href="#">
            <img
              class="avatar-large"
              :src="
                getUser(post).avatar.startsWith('http') ||
                getUser(post).avatar.startsWith('https')
                  ? getUser(post).avatar
                  : 'http://i.imgur.com/s8AzOkO.png'
              "
              :alt="getUser(post).username"
            />
          </a>
          <p class="desktop-only text-small">
            {{ getUser(post).postsCount }} posts
          </p>
        </div>
